...
app.get('/api/news/tags', (req, res)=>{
  ...
  const type=parseInt(tagId);
  const finalData=data.newsList.filter((news)=>{
    return news.type===type;
  })
  res.json({news:finalData});
})
...
app.post('/api/detail', async(req, res)=>{
  ...
  var t=parseInt(type);
  // console.log(data.newsList[id]);
  if(t==1){
    var newsInfo=data.newsList[index];
    // console.log(newsInfo); 
  }else{
    var newsInfo=data.swiperList[index];
    // console.log(newsInfo);
  }
